...
if __name__=='__main__':
    ...
    while CONTINUE_OUTLIER:
        ...
        while CONTINUE_QMAX: 
            ...
            while CONTINUE_Trans:
                try:
                    ## make input file with Json input for running bift
                    d.udpmessage({"_textarea":"prpoints %s\n" % prpoints})
                    d.udpmessage({"_textarea":"prpoints %s\n" % prpoints})
                    d.udpmessage({"_textarea":"prpoints %s\n" % prpoints})
                    d.udpmessage({"_textarea":"prpoints %s\n" % prpoints})
                    d.udpmessage({"_textarea":"prpoints %s\n" % prpoints})
                    ...
                    dmax_value = read_params(qmin,qmax)[1] # if ther is no parameters.dat file, this will give error
 ...
 #           alpha = '%f' % alpha_out # update logalpha value
            if n_pr <= 75:
                prpoints = '100'
            elif n_pr <= 110:
                prpoints = 150
