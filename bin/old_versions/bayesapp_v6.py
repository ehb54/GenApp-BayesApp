...
if __name__=='__main__':
    ...
    if logx:
        ...
        p1.plot(qdat,-5*np.ones(len(Idat)),linewidth=linewidth,linestyle='--',color='grey',zorder=2)
        p1.plot(qdat,5*np.ones(len(Idat)),linewidth=linewidth,linestyle='--',color='grey',zorder=3)
    else:
        p1.plot(qfit,Ifit*0,linewidth=linewidth,color='black',zorder=1)
        p1.plot(qfit,-5*np.ones(len(Ifit)),linewidth=linewidth,linestyle='--',color='grey',zorder=2)
        p1.plot(qfit,5*np.ones(len(Ifit)),linewidth=linewidth,linestyle='--',color='grey',zorder=3)
    idx = np.where(abs(R)>5)
