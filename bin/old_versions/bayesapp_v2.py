        ...
        while popen.poll() is None:
            for line in lines_iterator:
                ...
                if total_output_size > maximum_output_size:
                    ...
                    popen.terminate()
                    out_line = '\n\n!!!ERROR!!!\nProcess stopped - reached max time of 15 min (900 sec). Is data input a SAXS/SANS dataset with format (q,I,dI)?. If data is large (several thousand data points), consider rebinning the data. Or reduce number of points in p(r).\n\n'
                    d.udpmessage({"_textarea": out_line})
                    sys.exit()
                else:
                    out_line = '%s\n' % nline_latin
