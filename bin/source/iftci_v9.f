c************************************************************************************
c             Download of source code bift.f
c************************************************************************************
c
c   1) Compilation (linux):  gfortran -march=native -O3 bift.f -o bift
c                            -march=native may be left out 
c                            -m64 or -m32 may be added 
c                            ... depending on system
c
c   2) Run:                  bift < inputfile.d                    
c
c   3) The file: inputfile.d has to contain the 15 lines:
c                                                            input format
...
c   line 4:  d_max                         or a blank line   [float]*
c   line 5:  eta (non-dilute solutions)    or a blank line   [float]*
c   line 6:  alpha                         or a blank line   [float]*
c   line 7:  smearing constant             or a blank line   [float]
c   line 8:  ratio (non-dilute solutions)  or a blank line   [float]
c   line 9:  method (non-dilute solutions) or a blank line   [N]one or [M]oment or [E]vidence**
c   line 10: number of points in p(r)      or a blank line   [integer]
c   line 11: number of extra calc          or a blank line   [integer]
c   line 12: transformation                or a blank line   [D]ebye (default) or [N]egative or [M]axEnt or [B]essel or [S]ize
c   line 13: fit constant background       or a blank line   [Y]es or [N]o
c   line 14: non-const rescaling           or a blank line   [N]on-constant or [C]onstant
c   line 15: min points per Shannon bin    or a blank line   [integer]
c
...
c
c   ** None: fit only alpha and dmax, Moment: fit alpha, d_max and eta, Evidence: fit alpha, dmax, eta and ratio
c
c   NB NB The input values are the same as at the web site 
c   Only the first line has to be a non blank line
c   For further information see the website
...
C********************************************************************************
      ...
      REAL xori(nmax),sdori(nmax)
      REAL M(0:NMAX),YSUM(0:NMAX),FM(NMAX),w(nmax),dd(NMAX)
c      real ddx(1000),al(1000),etaa(1000),p(ndim),xi2(ndim,ndim)
      real ddx(NMAX),al(NMAX),etaa(NMAX),p(ndim),xi2(ndim,ndim)
...
c      hxname='in_'//bname
  ...
  132 FORMAT(1X,'qmax or <enter>                                 => ',$)
      ...
      ndata0=1
      WRITE(6,1329)
c rebin data if there are more than nrebin points
...
c      if(ndata.le.1) then
c      open(unit=88,file='plot2.pl',status='unknown')
c	write(88,*)'set nokey'
c	write(88,*)'set term gif'
c	write(88,*)'set output "fig2.gif"'
c       write(88,*)
c     -'set label " Wrong input format or q-limitation " at -4.8,0.55'
c       write(88,*)
c     -'set label "    No points read from data file   " at -4.5,0.45'
c	write(88,*)'set yrange [0:1]'
c	write(88,*)'plot 0'
c       write(88,*)'exit gnuplot'
c      close(88) 

c      open(unit=88,file='plot5.pl',status='unknown')
c 	write(88,*)'set nokey'
c	write(88,*)'set term gif'
c	write(88,*)'set output "fig5.gif"'
c       write(88,*)
c     -'set label " Wrong input format or q-limitation " at -4.8,0.55'
c       write(88,*)
c     -'set label "    No points read from data file   " at -4.5,0.45'
...
cccccccccccccccccccccccccccc changed feb 2012
c 1234 if(ndata0.eq.0) then
c      open(unit=88,file='plot2.pl',status='unknown')
c	write(88,*)'set nokey'
c	write(88,*)'set term gif'
c	write(88,*)'set output "fig2.gif"'
c       write(88,*)
c     -'set label "  Wrong input format in data file   " at -4.8,0.6'
c       write(88,*)
c     -'set label "   Zero standard deviation read     " at -4.8,0.5'
c       write(88,*)
c     -'set label "      Pls. check the data file      " at -4.5,0.4'
c	write(88,*)'set yrange [0:1]'
c	write(88,*)'plot 0'
c       write(88,*)'exit gnuplot'
c      close(88) 
...
c      endif
  ...
  138 FORMAT(1X,'Transformation 
     -[D]ebye (def), [B]essel, [C]osine, [S]ize, [N]eg. Debye => ',$)
      read(555,4)dummy
      answer5='D'
      if(dummy.ne.'       ') then
      open(unit=50,file='dummy.d',status='unknown')
...
c      if(nfit.eq.3.and.answerm.eq.'M') rprec=0.0001
c      if(nfit.eq.3.and.answerm.eq.'Q') rprec=0.0001
      if(nfit.eq.3.and.answerm.eq.'M') rprec=0.1
      if(nfit.eq.3.and.answerm.eq.'Q') rprec=0.1
...
c 1 is subtracted from nfit below
      ...
      WRITE(6,1138)
 1138 FORMAT(1X,'Fitting flat background [N]o or [Y]es (default) => ',$)
      read(555,4)dummy
      answer7='Y'
      if(dummy.ne.'       ') then
      open(unit=50,file='dummy.d',status='unknown')
      ...
      WRITE(6,1388)
 1388 FORMAT(1X,'Const. rescale (C) (default) or  non-const (N) => ',$)
      read(555,4)dummy
      answer8='C'
      if(dummy.ne.'       ') then
      open(unit=50,file='dummy.d',status='unknown')
 ...
 9134 FORMAT(1X,'Minimum number of points per Shannon bin => ',$)
      ...
      write(6,*)'Min Shannon bin size = ',nbin
...
c*********************************************
      ...
      countn=1.0
      do 111,i=1,mtotxx
c       qmax for ns'th shannon channel
        qn=ns*pi/Dmax
        res=(fm(20+i)-y(i))/sdori(i)
  ...
  111 continue
      do 112,i=1,n
        ...
        chi2rn=sumsquare(i)/(countn(i)-ngpartial)
        betan(i)=sqrt(chi2rn)
  112 continue
      n=1
      ns=1
...
c*********************************************
  ...
  956 format(1x,'I(0) estimated             : ',f9.2,
     -'  +- ',f8.2,' ')
      else
      write(166,986)fm1,sfm1
  986 format(1x,'I(0) estimated             : ',e9.3,
     -'  +- ',e8.3,' ')
      endif
 ...
 1959 format(1x,'Axial ratio from p(r) (pro): ',f9.2,
     -'  +- ',f8.2,' ')

      write(166,1960)ax2,sax2
 1960 format(1x,'Axial ratio from p(r) (obl): ',f9.2,
     -'  +- ',f8.2,' ')
 ...
 1962 format(1x,'Background estimated       : ',13x,i9)
  ...
  963 format(1x,'Volume fraction            : ',f9.2,
     -'  +- ',f8.2,' ')
      endif
      write(166,964)alp,salp
  964 format(1x,'Log(alpha) (smoothness)    : ',f9.2,
     -'  +- ',f8.2,' ')
  ...
  965 format(1x,'Evidence at maximum        : ',e9.2,
     -'  +- ',e9.2,' ')
      write(166,979)pval
  979 format(1x,'Probability of chi-square  : ',e9.2)
...
c****************************************
  ...
  947 format(1x,'Cpu time used         :  ',f4.1,
     -' seconds')
      ...
      x(1)=x(nn+1)
c      call plot(xf(ptot),f,ptot,fm,mtot,nadd,aname,x,cav,alp,dot,m)
c      open(unit=37,file='p.bat',status='unknown')
c      write(37,9997)plname
c 9997 format(1x,'wgnuplot ',a,' -')
      goto 9999
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
   ...
   75 CONTINUE
      ...
      fx=(2*alpha*m(i)/sigma(i)+ysum(i)-fsumi)/(2*alpha/sigma(i)+b(i,i))
        endif
...
c      if((itetot.ge.itetotmax).or.(cpu.ge.cpumax)) then

c      open(unit=88,file='plot1.pl',status='unknown')
c	write(88,*)'set term gif'
c	write(88,*)'set nokey'
c	write(88,*)'set output "fig1.gif"'
c       write(88,*)
c     -'set label "Too many iterations or max cpu time." at -6,0.55'
c       write(88,*)
c     -'set label "Try reducing the number of points." at -4.5,0.45'
c	write(88,*)'set yrange [0:1]'
c	write(88,*)'plot 0'
c       write(88,*)'exit gnuplot'
c      close(88)
...
c      open(unit=88,file='plot3.pl',status='unknown')
c	write(88,*)'set nokey'
c	write(88,*)'set term gif'
c	write(88,*)'set output "fig3.gif"'
c       write(88,*)
c     -'set label "Too many iterations or max cpu time." at -6,0.55'
c       write(88,*)
c     -'set label "Try reducing the number of points." at -4.5,0.45'
...
c      open(unit=88,file='plot4.pl',status='unknown')
c	write(88,*)'set nokey'
c	write(88,*)'set term gif'
c	write(88,*)'set output "fig4.gif"'
c       write(88,*)
c     -'set label "Too many iterations or max cpu time." at -6,0.55'
c       write(88,*)
c     -'set label "Try reducing the number of points." at -4.5,0.45'
...
c        close(88)
c      write(6,*)'error4',itetot,cpu,' > ',itetotmax,cpumax
...
c**********************************************************************
c      subroutine plot(dmax,f,ptot,fm,mtot,nadd,oname,x,chi,alpha,dot,m)
c      parameter (nmax=1000)
c      CHARACTER*18 oname
c      character*10 aname
c      chARACTER*20 plname,comment
c      CHARACTER*23 hxname,gsname,gxname,sname
c      CHARACTER*26 HNAME
c      character*40 screen
c      character*10 bname
c      character*5 fname,gname
c      character*4 gsname,gxname,sname
c      character*6 hxname
c      character*1 answer8,answer5,answer7,answerm
c      integer ptot,clockold

c      common /data11/ itetot,itetotmax
c      common /data15/ answer5
c      common /data25/ csmear,cexp
c      common /back/ background
c      common /screen/ answer8,answer7
c      common /cputime/ clockold,cpumax,answerm
c      common /rescale/ nbin

c      real xf(0:nmax),f(0:nmax),fm(nmax),x(nmax),m(0:nmax)
c      plname='pl'//aname
c      aname=oname
...
c      hxname='data.d'
c      sname='st.d'
c      if(answer8.eq.'N') screen='set term gif medium size 500,360'
c      if(answer8.eq.'C') screen='set term gif large size 1000,720'
...
c      fmin=1.e20
c      do 10 j=1,ptot
c      if(f(j).gt.fmax) fmax=f(j)
c      if(f(j).le.fmin) fmin=f(j)
c  10  continue
c      exclmin=0
c      do 11 j=ptot+1,2*ptot
c      if(f(j).lt.exclmin) exclmin=f(j)
c  11  continue
c      fmmin=1.e6
c      fmmax=0
c      do 12 i=1,mtot-nadd
c      if(fm(i).lt.fmmin) fmmin=fm(i)
c      if(fm(i).gt.fmmax) fmmax=fm(i)
c  12  continue

c      if(dot.ge.0.9) comment='Conv. OK'

c      if(dot.le.0.9) comment='NOT OK !'

c      mt=mtot-nadd
c      fratiox=(fm(1)+fm(2)+fm(3))/(fm(mt-2)+fm(mt-1)+fm(mt))
c      fratiox=abs(fratiox)
c      write(6,*)
...
c	write(88,*)'set noclip points'
c	write(88,*)'set clip one'
c	write(88,*)'set noclip two'
c	write(88,*)'set bar 1.000000'
c	write(88,*)'set xdata'
c	write(88,*)'set ydata'
c	write(88,*)'set zdata'
c	write(88,*)'set x2data'
c	write(88,*)'set y2data'
c	write(88,*)'set boxwidth'
c	write(88,*)'set dummy x,y'
c	write(88,*)'set format x "%g"'
c	write(88,*)'set format y "%g"'
c	write(88,*)'set format x2 "%g"'
c	write(88,*)'set format y2 "%g"'
c	write(88,*)'set format z "%g"'
c	write(88,*)'set angles radians'
c	write(88,*)'set nogrid'
c	write(88,*)'set key title ""'
c	write(88,*)'set nolabel'
c	write(88,*)'set noarrow'
c	write(88,*)'set nolinestyle'
c	write(88,*)'set nologscale'
c	write(88,*)'set offsets 0, 0, 0, 0'
c	write(88,*)'set pointsize 1'
c	write(88,*)'set encoding default'
c	write(88,*)'set nopolar'
c	write(88,*)'set noparametric'
c	write(88,*)'set view 60, 30, 1, 1'
c	write(88,*)'set samples 100, 100'
c	write(88,*)'set isosamples 10, 10'
c	write(88,*)'set surface'
c	write(88,*)'set nocontour'
c	write(88,*)'set clabel ','''%8.3g'''
c	write(88,*)'set mapping cartesian'
c	write(88,*)'set nohidden3d'
c	write(88,*)'set cntrparam order 4'
c	write(88,*)'set cntrparam linear'
c	write(88,*)'set cntrparam levels auto 5'
c	write(88,*)'set cntrparam points 5'
c	write(88,*)'set data style points'
c	write(88,*)'set function style lines'
c	write(88,*)'set xzeroaxis lt -2 lw 1.000'
c	write(88,*)'set x2zeroaxis lt -2 lw 1.000'
c	write(88,*)'set yzeroaxis lt -2 lw 1.000'
c	write(88,*)'set y2zeroaxis lt -2 lw 1.000'
c	write(88,*)'set tics in'
c	write(88,*)'set ticslevel 0.5'
c	write(88,*)'set ticscale 1 0.5'
c	write(88,*)'set mxtics default'
c	write(88,*)'set mytics default'
c	write(88,*)'set mx2tics default'
c	write(88,*)'set my2tics default'
c	write(88,*)'set xtics border mirror norotate autofreq' 
c	write(88,*)'set ytics border mirror norotate autofreq' 
c	write(88,*)'set ztics border nomirror norotate autofreq' 
c	write(88,*)'set nox2tics'
c	write(88,*)'set noy2tics'
c	write(88,*)'set rrange [ * : * ] noreverse nowriteback'  
c	write(88,*)'set trange [ * : * ] noreverse nowriteback'  
c	write(88,*)'set urange [ * : * ] noreverse nowriteback'  
c	write(88,*)'set vrange [ * : * ] noreverse nowriteback'
c	write(88,*)'set xrange [ * : * ] noreverse nowriteback'  
c	write(88,*)'set x2range [ * : * ] noreverse nowriteback'  
c	write(88,*)'set yrange [ * : * ] noreverse nowriteback' 
c	write(88,*)'set y2range [ * : * ] noreverse nowriteback'
c	write(88,*)'set zrange [ * : * ] noreverse nowriteback' 
c	write(88,*)'set zero 1e-008'
c	write(88,*)'set lmargin 0'
c	write(88,*)'set bmargin 0'
c	write(88,*)'set rmargin 0'
c	write(88,*)'set tmargin 0'
c	write(88,*)'set nolog'
c	write(88,*)'set nolabel'
c	write(88,*)'set nokey'
c	write(88,*)'reset'
...
c	write(88,*)'set yrange [0:*]'
c	write(88,*)'set xrange [*:',xmax,']'


c       xmin=x(1)
c       xmax=x(mtot)
c       if(x(1).ge.x(mtot))then
c       xmax=x(1)
c       xmin=x(mtot)
c       endif
c        write(88,744)aname,xmin,xmax
c 744  format(1x,'set title "S(q) for ',a,' from ',f8.4,' to ',f8.4,'"')

c	write(88,*)'set xlabel "q"  '
c	write(88,*)'set ylabel "S(q)" '

c	write(88,*)'set xtics 1'
c      if(xmax.le.4) write(88,*)'set xtics 0.5'
c	if(xmax.le.1.6) write(88,*)'set xtics 0.2'
c	if(xmax.le.0.5) write(88,*)'set xtics 0.1'
c	if(xmax.le.0.26) write(88,*)'set xtics 0.05'
c	if(xmax.le.0.16) write(88,*)'set xtics 0.02'
c	if(xmax.le.0.08) write(88,*)'set xtics 0.01'
c	if(xmax.le.0.04) write(88,*)'set xtics 0.005'
c	if(xmax.le.0.016) write(88,*)'set xtics 0.002'
c	if(xmax.le.0.005) write(88,*)'set xtics 0.001'
	
c  818 format(a)
c      write(88,818)screen
c	write(88,*)'set output "fig1.gif"'
c	write(88,881)sname
c  881 format(1x,'plot 1 w l 3,''',a,''' w l 1') 
c        write(88,*)'exit gnuplot'
c        close(88)

c**********************'' plot 2 ******************************************
c       xl=80
c       if(fratiox.ge.xl) open(unit=88,file='plot2.pl',status='unknown')
c       if(fratiox.lt.xl) open(unit=88,file='plot3.pl',status='unknown')

c	write(88,*)'set yrange [*:*]'
c	write(88,*)'set xrange [*:*]'
c	write(88,*)'set logsc y'
c	write(88,*)'set xlabel "q"  0.0,0.0'
c	write(88,*)'set ylabel ','''I(q)  [a.u.]''',' 0.0'

c      if(itetot.ge.itetotmax) then
c      write(88,*)'set label "No convergence" at 0.5,0.5'
c      endif

c	write(88,*)'set nokey'
c      if(fmmin.le.0) fmmin=1.e-5
c	if(xmax.le.1) write(88,*)'set xtics 0.1'
c	if((xmax.ge.1).and.(xmax.le.4)) write(88,*)'set xtics 0.5'
c	if(xmax.gt.4) write(88,*)'set xtics 1'
c	write(88,*)'set  yrange [',0.8*fmmin,':',2*fmmax,']'
c	write(88,*)'set  xrange [0:',xmax,']'

c	write(88,*)'set xtics 1'
c      if(xmax.le.4) write(88,*)'set xtics 0.5'
c	if(xmax.le.1.6) write(88,*)'set xtics 0.2'
c	if(xmax.le.0.5) write(88,*)'set xtics 0.1'
c	if(xmax.le.0.3) write(88,*)'set xtics 0.05'
c	if(xmax.le.0.16) write(88,*)'set xtics 0.02'
c	if(xmax.le.0.08) write(88,*)'set xtics 0.01'
c	if(xmax.le.0.04) write(88,*)'set xtics 0.005'
c	if(xmax.le.0.016) write(88,*)'set xtics 0.002'
...
c       xmin=x(mtot)
c       endif
c        write(88,444)aname,xmin,xmax
c 444  format(1x,'set title "data + fit ',a,' from',f8.4,' to',f8.4,'"')

c      if((ptot.lt.90).and.(fratiox.ge.xl*5.)) then
c      write(88,476)"Try to increase the number",xmax*0.4,1.05*fmmax*0.8
c      write(88,476)"of points used for p(r) or",xmax*0.4,1.05*fmmax*0.4
c      write(88,476)"to reduce the used q-range",xmax*0.4,1.05*fmmax*0.205
c 476  format(1x'set label "',a,'" at ',e8.2,',',e8.2)
c      endif

c      write(88,818)screen

c       if(fratiox.ge.xl) write(88,*)'set output "fig2.gif"'
c       if(fratiox.lt.xl) write(88,*)'set output "fig3.gif"'
c	write(88,882)hxname,fname,fname
c  882 format(1x,'plot ''',a,''' w e 1,''',a,''' w l 2,''',a,''' 
c     -using ','''%lf %*lf %lf''',' w l 3')
c        write(88,*)'exit gnuplot'
c        close(88)

c**********************'' plot 3 ******************************************

c       if(fratiox.ge.xl) open(unit=88,file='plot3.pl',status='unknown')
c       if(fratiox.lt.xl) open(unit=88,file='plot2.pl',status='unknown')
...
c	write(88,*)'set ylabel ','''I(q)  [a.u.]''',' 0.0'
c	write(88,*)'set nokey'
c	write(88,*)'set  yrange [1.e-8:',fmmax*1.05,']'
c	write(88,*)'set  xrange [0:',xmax,']'
c      write(88,818)screen
...
c       if(fratiox.ge.xl) write(88,*)'set output "fig3.gif"'
c       if(fratiox.lt.xl) write(88,*)'set output "fig2.gif"'
c      	write(88,882)hxname,fname,fname
c        write(88,*)'exit gnuplot'
c        close(88)
...
c	write(88,*)'set nokey'
c       dmax=3*dmax
c	if(dmax.le.10) write(88,*)'set xtics 4'
c	if((dmax.gt.10).and.(dmax.le.20)) write(88,*)'set xtics 5'
c	if((dmax.gt.20).and.(dmax.le.50)) write(88,*)'set xtics 10'
c	if((dmax.gt.50).and.(dmax.le.100)) write(88,*)'set xtics 20'
c	if((dmax.gt.100).and.(dmax.le.500)) write(88,*)'set xtics 100'
c	if(dmax.gt.500) write(88,*)'set xtics 100'
c	if(dmax.gt.1000) write(88,*)'set xtics 200'
c	if(dmax.gt.2000) write(88,*)'set xtics 500'
c	if(dmax.gt.5000) write(88,*)'set xtics 1000'
c       dmax=0.3333*dmax

c	write(88,*)'set nolabel'
c	write(88,*)'set nologsc'
c	write(88,*)'set ylabel "p(r)  [a.u.]" 0.0,0.0'
c	write(88,*)'set xlabel ','''r''',' 0.0,0.0'
c	write(88,*)'set xrange [0:',2.5*dmax,']'
c	write(88,*)'set yrange [',1.05*exclmin,':',1.05*fmax,']'
c	write(88,*)'set nokey'

c*******************
c      write(88,574)aname,chi,alpha

c      write(88,575)comment,dmax*1.4,1.05*fmax*0.9
c*****************

c      write(88,818)screen
c	write(88,*)'set output "fig4.gif"'
c	write(88,885)bname,gsname,gxname,bname,gsname,gxname
...
c        open(unit=88,file='plot5.pl',status='unknown')
c	write(88,*)'set nolabel'
c	if(dmax.le.10) write(88,*)'set xtics 1'
c	if((dmax.gt.10).and.(dmax.le.20)) write(88,*)'set xtics 2'
c	if((dmax.gt.20).and.(dmax.le.50)) write(88,*)'set xtics 5'
c	if((dmax.gt.50).and.(dmax.le.100)) write(88,*)'set xtics 10'
c	if((dmax.gt.100).and.(dmax.le.500)) write(88,*)'set xtics 50'
c	if(dmax.gt.500) write(88,*)'set xtics 100'
c	if(dmax.gt.1000) write(88,*)'set xtics 200'
c	if(dmax.gt.2000) write(88,*)'set xtics 1000'
c	if(dmax.gt.5000) write(88,*)'set xtics 2000'
c	write(88,*)'set nologsc'
      
c      if(answer5.eq.'D') then
c	write(88,*)'set ylabel "p(r)  [a.u.]" 0.0,0.0'
c	write(88,*)'set xlabel ','''r''',' 0.0,0.0'
c      endif
 
c      if(answer5.eq.'K') then
c	write(88,*)'set ylabel "G(l)*l**4  [a.u.]" 0.0,0.0'
c	write(88,*)'set xlabel ','''l''',' 0.0,0.0'
c      endif
 
c      if(answer5.eq.'B') then
c	write(88,*)'set ylabel "Cross Section distr.  [a.u.]" 0.0,0.0'
c	write(88,*)'set xlabel ','''r (cross section)''',' 0.0,0.0'
c      endif
 
c      if(answer5.eq.'C') then
c	write(88,*)'set ylabel "Thickness distr.  [a.u.]" 0.0,0.0'
c	write(88,*)'set xlabel ','''r (thickness)''',' 0.0,0.0'
c      endif

c      if(answer5.eq.'S') then
c	write(88,*)'set ylabel "Sizedistribution  [a.u.]" 0.0,0.0'
c      write(88,*)'set xlabel ','''R (number weighted) ''',' 0.0,0.0'
c      endif       

c         if(answerm.eq.'M'.or.answerm.eq.'Q') then
c       	write(88,*)'set xrange [0:',1.05*dmax,']'
c              else
c       	write(88,*)'set xrange [0:',dmax,']'
c         endif
c	write(88,*)'set yrange [',0,':',1.05*fmax,']'
c       if((answer5.eq.'K').or.(answer5.eq.'N').or.answer5.eq.'C'.
c     -or.answerm.eq.'M') then
c         if(fmin.ge.0.or.abs(fmin).le.0.005*fmax) then
c         write(88,*)'set yrange [',0.,':*]'
c         write(88,*)'set yrange [',0.,':',1.05*fmax,']'
c         else
c	write(88,*)'set yrange [*:*]'
...
c      if(answer5.ne.'S') then
c      write(88,574)aname,chi,alpha
c 574  format(1x,'set title "p(r) ',
c     -'',a,' chi = ',f7.3,' log(alpha) =',f6.1,' ',a,'"')
c      else
c      write(88,5744)aname,chi,alpha
c 5744 format(1x,'set title "Size ',
c     -'',a,' chi = ',f7.3,' log(alpha) =',f6.1,' ',a,'"')     
c      endif
c      if(answer5.eq.'K') then
c      write(88,597)aname,chi,alpha
c 597  format(1x,'set title "G(l)*l**4  ',
c     -'',a,' chi = ',f7.3,' log(alpha) =',f6.1,' ',a,'"')
c      endif
...
c      if(answer7.eq.'Y') 
c     -write(88,576)"backgr = ",background,dmax*0.54,1.05*fmax*0.8
c 576  format(1x'set label "',a,e9.3,'" at ',e8.2,',',e8.2)
c      write(88,575)comment,dmax*0.72,1.05*fmax*0.9
c 575  format(1x'set label "',a,'" at ',e8.2,',',e8.2)
c      if((f(1).gt.0.85*f(2)).and.(answer7.ne.'Y')) 
c     -write(88,577)"Fit backgr ?",dmax*0.72,1.05*fmax*0.8
c 577  format(1x'set label "',a,'" at ',e8.2,',',e8.2)
...
c	write(88,*)'set output "fig5.gif"'
c       if((answer5.eq.'K').or.(answer5.eq.'N').or.answer5.eq.'C') then
c	write(88,898)bname,bname
c 898  format
c     -(1x,'plot ''',a,''' w e 1,''',a,''' w l 1, 0 w l 3')
c       else
c         if(answerm.eq.'M'.or.answerm.eq.'Q') then
c	write(88,887)bname,bname,'prior.d'
c 887  format(1x,'plot ''',a,''' w e 1,''',a,''' w l 1,''',a,''' w l 2,
c     -0 w l 3')
c         goto 8888
c         endif
c	write(88,888)bname,bname
c 888  format(1x,'plot ''',a,''' w e 1,''',a,''' w l 1')
c 8888 continue
c       endif
...
c	write(88,*)'set term gif'
c	write(88,*)'set output "fig2.gif"'
c       write(88,*)'set label "NO USABLE ALPHA/DIAMETER" at -3.5,0.55'
c       write(88,*)
c     -'set label "Try entering estimates" at -3.2,0.45'
c	write(88,*)'set yrange [0:1]'
c	write(88,*)'plot 0'
c       write(88,*)'exit gnuplot'
c      close(88) 
...
c	write(88,*)'set term gif'
c	write(88,*)'set output "fig5.gif"'
c       write(88,*)'set label "NO USABLE ALPHA/DIAMETER" at -3.5,0.45'
c       write(88,*)
c     -'set label "Try entering estimates" at -3.2,0.45'
c	write(88,*)'set yrange [0:1]'
c	write(88,*)'plot 0'
...
c	write(88,*)'set output "fig2.gif"'
c       write(88,*)
c     -'set label " Sorry - something went wrong ...     " at -4.8,0.5'
c       write(88,*)
c     -'set label "Perhaps empty spaces in the file name?" at -4.8,0.4'
c	write(88,*)'set yrange [0:1]'
c	write(88,*)'plot 0'
c       write(88,*)'exit gnuplot'
c      close(88) 
...
c      PARAMETER (ITMAX=100000,EPS=30,FPMIN=1.e-30)
      INTEGER i
