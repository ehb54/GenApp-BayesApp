...
c*********************************************
       ...
       sum_all=0
       sigma_rt=var_rt**0.5
       xmin = av_rt-6.0*sigma_rt
       kmax = 100
       xstep = 2.*6.*sigma_rt/kmax
       kloop: do k=1,kmax
         x_k=xmin+k*xstep
         d_rt=exp(-(av_rt-x_k)**2.0/var_rt/2.0)
         sum_all=sum_all+d_rt
         write(6,*)'rt,av,var_rt=',runtotav,av_rt,var_rt
c         write(6,*)'d_rt,x_k,rt,mu,sum_all =',d_rt,x_k,runtotav,mu_rt,sum_all
         if(x_k.ge.runtotav)then
           sum_rt=sum_rt+d_rt
         endif
       end do kloop
       pval_runtot = sum_rt/sum_all
       write(6,*)'sum_rt,sum_all,pval = ',sum_rt,sum_all,pval_runtot
