...
c************************************************************************************
c
c   Written by:              Steen Hansen
c   With contributions from: Andreas Haahr Larsen
c                            Martin Cramer Pedersen
c
...
c 
      ...
      WRITE(6,139)
c  139 FORMAT(1X,'Number of points used for p(r)    (50-100)     => ',$)
  139 FORMAT(1X,'Number of points used for p(r)    (20-500)     => ',$)
      read(555,4)dummy
c      ntot=50
      ntot=70
      if(dummy.ne.'       ') then
      ...
      if(ntot.gt.500)  ntot=500
      if(ntot.lt.20)   ntot=20

c      if((etaest.gt.0).and.(ntot.gt.80)) cpumax=900
...
c******************************************************************** 
      ...
      if(ndata.lt.5) then
        write(6,*)''
        write(6,*)'!!Problem!'
        write(6,*)' Number of points less than 5'
        write(6,*)' - try extending the q range'
        write(6,*)' - check file format: (q,I,sigma)'
        write(6,*)' - check units: nm-1 or A-1'
        write(6,*)''
        STOP
      endif
...
c      OPEN(36,FILE=KNAME,STATUS='unknown',access='append')
c      nextra=ntot
c      write(36,6670)'-----------------------------------------
c     ------------------------------------'
c      write(36,6668)'Input values:   ',qmin,qmax,dest,
c     -etaest,alphaest,ratio,answer,ntot,rprec,nerror
c 6668 format(a,f6.4,x,f6.4,x,f6.2,x,f6.4,x,f7.2,x,f6.3,x,a,x,i3,
c     -x,f7.4,x,i3)
c      write(36,*)
 6669 write(6,*)'                                          '
      write(6,*)'                                          '
...
c        Radius=5.0
c        volfrac=0.06
c        StructFact=Sq(x(i),Radius,volfrac)        
c        WRITE(11,*)X(I),StructFact*30.0
c        WRITE(11,*)X(I),StructFact*fm(i)
        WRITE(11,*)X(I),fm(i)
...
c      if(answer5.eq.'D') 
      ...
      if(f(0).eq.0) then
c      write(36,*)' Rg  ax ratio  -K     chi^2  log(alpha)  eta   dmax 
c     - I(0)  evidence'
      write(6,*)' Rg  ax-ratio  -K     chi^2  log(alpha)  eta   dmax 
     - I(0)  evidence'
      endif
      if(f(0).ne.0) then
c      write(36,*)' Rg  ax ratio backg   Chi  log(alpha)   eta      d 
c     - I(0)  evidence'
     ...
     - I(0)  evidence'
      ...
      write(6,*)
c      write(36,2135)rg,ax,-S,CAV,alp,eTAav,dmax,fm1,-evimax
c      write(36,2134)srg,sax,sS,sCAV,salp,seTAav,sdmax,sfm1
c     -,sevi
c 2133 format
c     -(f5.2,1x,F6.2,1X,f8.5,1x,
c     -f6.3,1x,f8.3,1x,f6.3,1x,f6.2,1x,f7.3,1X,f9.2)
 2134 format
...
c      do 998 i = 1,mtotxx
      do 998 i = 1,20
      sumtot=sumtot+probbi(mtotxx,i)
      if(i.ge.istart) then
c      if(i.ge.10) then
      sumpart=sumpart+probbi(mtotxx,i)
      endif
 998  CONTINUE 
...
c      CALL SUB1(pval_runmax,mtotxx,nrunmaxav,SUB1)
      CALL SUB1(anr,25,4,SUB1)
      pval_runmax=anr/2**25
...
c  978 format(1x,'Number of Shannon channels, qrange*(dmax/pi): ',f9.2)
  ...
  982 format(1x,'Longest run                : ',f9.2)
      write(166,984)sumpart
  984 format(1x,'Prob., longest run (cormap): ',e9.4)
      write(166,988)sumtot 
  988 format(1x,'Prob., longest run (cormap): ',e9.4)
  ...
  987 format(1x,'Prob.,  number of run      : ',e9.2)
      write(166,*)
      call system_clock(clock)
      cpu=(clock-clockold)*0.001
      write(166,955)cpu,cpumax
  955 format(1x,'CPU time used: ',f6.1,
     -' seconds of maximum ',f6.1,' seconds')
      else
...
c      write(137,*)cav
c      write(137,*)rtio
c      write(137,*)background
c      write(137,*)qmin
c      write(137,*)qmax 
c      write(137,*)smear1
c      write(137,*)smear2
c      write(137,4)answer5
c      close(137)
...
c     -1X,F10.4,1x,f7.3,2x,f7.4,1x,e8.1,1x,f12.1)
  161 FORMAT(1X,I4,1x,f7.2,1X,f10.4,
     -1X,F10.4,1x,f10.4,1x,f7.3,2x,f7.4,1x,e8.1,1x,f12.1)
...
c **********************************************************************
      ...
      FUNCTION Sq(q,R,eta)
c     USES function G(A,eta)
c     q is a scalar (vector operation not possible - use a loop)
      REAL q,R,eta
      REAL A,Gvalue,Sq
      if(eta.gt.0.0)then
          A=2.0*R*q
          Gvalue=G(A,eta)
          Sq=1.0/(1.0+24.0*eta*Gvalue/A)
      else
          Sq=1.0
      endif
c      Sq=A
c      Sq=2.0*R*q
      ...
      FUNCTION G(A,eta)
      REAL A,eta,G
      REAL aa,b,c,sinA,cosA,fa,fb,cc,fc
      aa = (1.0+2.0*eta)**2.0/(1.0-eta)**4.0
      b = -6.0*eta*(1.0+eta/2.0)**2.0/(1.0-eta)**4.0
      c = eta*aa/2.0
      sinA = sin(A)
      cosA = cos(A)
      fa = sinA-A*cosA
      fb = 2.0*A*sinA+(2.0-A**2.0)*cosA-2.0
      cc = (3.0*A**2.0-6.0)*cosA+(A**3.0-6.0*A)*sinA+6.0
      fc = -A**4.0*cosA + 4.0*cc
      G = aa*fa/A**2.0 + b*fb/A**3.0 + c*fc/A**5.0
      return
      END
...
c      if (n.lt.0) pause 'negative factorial in factln?'
      ...
      FUNCTION probbi(n,k)
c     binomial probability when p=q=0.5
      INTEGER k,n
      REAL probbi
      probbi=bico(n,k)*0.5**n
      return
      END

      RECURSIVE FUNCTION funcA(n,runmax) result(anr)
c     Schilling distribution, from Schilling1990:
c     https://www.maa.org/sites/default/files/images/upload_library/22/Polya/07468342.di020742.02p0021g.pdf
c     n is number of data points, runmax is longest run
      INTEGER n,runmax
c     anr is number of combinations with n and runmax
      REAL anr
      if (n.le.runmax) then
      anr=2**n
      else
...
c      Ndata=mtotxx
      ...
      SUBROUTINE SUB1(anr,Ndata,runmax,DUMSUB)
c     Schilling distribution, from Schilling1990:
c     https://www.maa.org/sites/default/files/images/upload_library/22/Polya/07468342.di020742.02p0021g.pdf
c     n is number of data points, runmax is longest run
      INTEGER Ndata,runmax
      EXTERNAL DUMSUB
c     anr is number of combinations with Ndata and runmax
      REAL tmp,anr
c      write(6,*)'SUB1 call, Ndata = ',Ndata
      if (Ndata.le.runmax) then
      anr=2**Ndata
      else
      sum=0
      DO 123 i=0,runmax
      CALL DUMSUB(tmp,Ndata-1-i,runmax,DUMSUB)
      sum=sum+tmp
 123  CONTINUE
      anr=sum
      endif
      ...
      FUNCTION PROBNR(n,runmax)
c     https://www.nature.com/articles/nmeth.3358#Sec2      
      INTEGER n,runmax
      real p,A
      CALL SUB1(A,n,runmax,SUB1)
c      p=1.0-A/2**n
      p=A
      return
      END
